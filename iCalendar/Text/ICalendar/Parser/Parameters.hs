...
parseDuration what txt =
    case runParser dur def what txt of
         Left e -> throwError $ "Invalid duration: "
                              ++ unlines [show txt, show e]
         Right x -> return x
  ...
  where freq = istring "FREQ=" *> frequency
        ...
        until' = do txt <- manyTill P.anyChar (void (P.char ';') <|> P.eof)
                    return . Just . Left $
                        case dts of
                             DTStartDateTime _ _ ->
                                 Right <$> parseDateTime Nothing (T.pack txt)
                             DTStartDate _ _ ->
                                 Left <$> parseDate (T.pack txt)
...
parseUTCPeriod :: Text -> ContentParser UTCPeriod
parseUTCPeriod txt = do
    let (dateTime', x) = T.drop 1 <$> T.break (=='/') txt
    when (T.null x) . throwError $ "Invalid UTCperiod: " ++ show txt
...
parsePeriod :: Maybe Text -> Text -> ContentParser Period
parsePeriod tzid txt = do
    let (dateTime', x) = T.drop 1 <$> T.break (=='/') txt
    when (T.null x) . throwError $ "Invalid period: " ++ show txt
